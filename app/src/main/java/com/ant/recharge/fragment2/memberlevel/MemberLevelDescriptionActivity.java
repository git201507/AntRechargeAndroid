...
public class MemberLevelDescriptionActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ((TextView)findViewById(R.id.text_title)).setText("会员等级说明");

        String userStr = readPreferences(NetLoginInterface.ANT_LOGIN_USER, "additional");
        try {
            User user = JsonUtil.decode(userStr, User.class);
            webView.loadUrl(Profile.SERVER_ADDRESS + "wx/front/wdzh/levels.jhtml?memberId=" + user.getLoginName());
//            webView.loadUrl(Profile.SERVER_ADDRESS + "wx/front/wdzh/appLevels.jhtml?memberId=" + user.getLoginName());
        } catch (Exception e) {
            e.printStackTrace();
        }
